...
// immediately below for where the original text began.
    ...
    //
    // Track 1 Stage 2 (wobbly-honking-cascade.md, generalizing past the
    // single (An) case a real 68030 wouldn't show a gap for either):
    // plain absolute addressing ((xxx).W/(xxx).L, dec_abs_ea_en) needs
    // NO register read at all -- dec_abs_ea_val is a decode-time
    // constant, confirmed mutually exclusive with dec_is_idx in decode
    // (different EA mode-field values, so it never needs ex_xn_scaled
    // either). preview_is_abs is the new, independent, zero-port-cost
    // sibling to the (An)-based case below.
    //
    // Track 1 Stage 3: preview_is_an now ALSO covers `(d16,An)`, not just
    // literal `(An)` -- dropped the `dec_ea_offset==0` requirement Stage
    ...
    //
    // The Track 3 preview mechanism itself (reg_hazard, every family's
    // own *_final_ack/*_hazard signal, preview_current_ready, and
    // preview_ok) now lives in eu_seq_preview.svh, `` `include ``d from
    // eu_seq.sv immediately after this file -- extracted as a post-
    // Track-3 cleanup pass (pure text relocation, no behavior change).
    // Everything below this point (the mem_req/mem_rw/mem_wdata/mem_addr
    // dispatch mux) stays here: it predates Track 3 and serves every
    // special FSM's own bus-active flag, not just preview, so it isn't a
    // clean, self-contained extraction candidate the way the preview
    // logic above it was.
    ...
    assign mem_addr  = preview_ok     ? preview_addr :
