...
//
// Loads a hex file at runtime via +hexfile= plusarg, runs until STOP seen,
// logs bus cycles in the same format as cosim73 for buscmp.py comparison.
    ...
    initial begin
        ...
        if (check_d0)
            check({grpname, " D0 correct"},
                  u_top.u_eu.u_rf.d_reg[0] == exp_d0[31:0]);

        if (fail_count == 0) $display("PASS  cosim_%s", grpname);
        else                 $display("FAIL  cosim_%s (%0d)", grpname, fail_count);
        $finish;
    end
