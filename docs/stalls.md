...
**Coverage depth**: fixed and tested against 17 FSM sources so far — CAS2 (Phase 105,
...
5, Phase 189), and MOVE16, ABCD, SBCD, CMP2, CHK2, MOVE mem-mem, RTR, and RTE
(pipeline-stall breadth extension plan, elegant-gliding-fog.md Stages 1-3), via the
shared `run_int_mid_test` task, chosen to span the RMW-lock, byte-interleaved,
dual-address-predecrement, burst, two-read-bounds-check, and control-transfer/
stack-restore FSM shapes. The mechanism is decode-content-agnostic (it gates
...
~5 `ex_mem_stall` sources (PFLUSH/PTEST/PMOVE64) haven't been individually spot-checked
the way BERR-abort (Category I) was.
...
| F. Interrupt dispatch | `tb/stall_fsm_tb.sv` | Level-7 NMI mid-instruction, 17 sources (CAS2/MOVEM/memory-indirect EA/TAS/MOVEP/CAS/ADDX/PACK/BFINS/MOVE16/ABCD/SBCD/CMP2/CHK2/MOVEmm/RTR/RTE, Phases 105/125/126/189, elegant-gliding-fog.md Stages 1-3); non-idempotent dependent-instruction marker (regression would show up as a doubled value); exact bus-cycle count before the interrupt was recognized |
...
- **Interrupt-mid-FSM** (Category F) has 17 of ~19-23 possible FSM sources checked
  ...
  MOVEmm/RTR/RTE, added by the pipeline-stall breadth extension plan's own Stages 1-3,
  ...
  Remaining: PFLUSH/PTEST/PMOVE64.
