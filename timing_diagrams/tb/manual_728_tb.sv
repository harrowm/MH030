...
// Timing-diagram source testbench: MC68030UM.pdf Figure 7-28's own
// Long-Word Operand Write -- 16-Bit Port (tests/timing_manual_728.s
// header). Same hybrid-memory-region shape as manual_722/723_tb.sv:
// ordinary 32-bit inline model serves code/vectors/stack, a narrow
// $100-$1FF region (routed to tb/mem_model.sv, PORT_WIDTH=16) serves the
// target write.

module manual_728_tb;
    ...
    initial begin
        ...
        $readmemh("../tests/timing_manual_728.hex", rom);
    end
    ...
    initial begin
        $dumpfile("manual_728.vcd");
        $dumpvars(0, manual_728_tb);

        rst_n = 0;
        repeat(20) @(posedge clk_4x);
        #1; rst_n = 1;

        // Marker word index 64 ($100) -- watch for the write to land.
        for (int t = 0; t < 3000 && u_mem16.mem[64] !== 32'h1234_5678; t++)
            @(posedge clk_4x);

        repeat(8) @(posedge clk_4x);

        $display("FINAL mem[64] = %08x", u_mem16.mem[64]);
        $finish;
    end
