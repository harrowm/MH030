...
// Timing-diagram source testbench: MC68030UM.pdf Figure 7-30's own
// Asynchronous Byte Read-Modify-Write Cycle -- 32-Bit Port (TAS's own
// locked read-then-write, tests/timing_manual_730.s header). Mirrors
// manual_725_tb.sv's own inline-32-bit-model shape exactly (RMC/bus-lock
// timing doesn't involve dynamic sizing, so no mem_model.sv needed here).

module manual_730_tb;
    ...
    initial begin
        ...
        $readmemh("../tests/timing_manual_730.hex", rom);
    end
    ...
    initial begin
        $dumpfile("manual_730.vcd");
        $dumpvars(0, manual_730_tb);
