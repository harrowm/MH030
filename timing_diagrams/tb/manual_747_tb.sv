...
// Timing-diagram source testbench: MC68030UM.pdf Figure 7-47's own
// Breakpoint Acknowledge Cycle Timing (tests/timing_manual_747.s
// header). BKPT's own CPU-space read is just an ordinary /DSACK-
// terminated read at a fixed address (bkpt_num*4) -- the same inline
// memory model every other diagram in this set uses already answers
// it correctly, no special-case responder needed (mirrors
// tb/stall_fsm_tb.sv's own established BKPT-live-substitution
// convention exactly).

module manual_747_tb;
    ...
    initial begin
        ...
        $readmemh("../tests/timing_manual_747.hex", rom);
        rom[16'h3010 >> 2] = 32'hDEAD_BEEF;
    end
    ...
    initial begin
        $dumpfile("manual_747.vcd");
        $dumpvars(0, manual_747_tb);

        rst_n = 0;
        repeat(20) @(posedge clk_4x);
        #1; rst_n = 1;

        // MOVEQ #42,D0's own substituted execution proves the whole
        // BKPT-acknowledge sequence completed correctly.
        for (int t = 0; t < 3000 && u_top.u_eu.u_rf.d_reg[0] !== 32'd42; t++)
            @(posedge clk_4x);

        repeat(20) @(posedge clk_4x);

        $display("FINAL d_reg[0] = %0d", u_top.u_eu.u_rf.d_reg[0]);
        $finish;
    end
