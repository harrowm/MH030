...
// Timing-diagram source testbench: MC68030UM.pdf Figure 7-25's own
// Read-Write-Write-Read longword chain (zero-wait-state portion only --
// see tests/timing_manual_725.s's own header), driven through the REAL
// EU/decode pipeline. Mirrors tb/read_cycle_eu_tb.sv exactly.

module manual_725_tb;
    ...
    initial begin
        ...
        $readmemh("../tests/timing_manual_725.hex", rom);
    end
    ...
    initial begin
        $dumpfile("manual_725.vcd");
        $dumpvars(0, manual_725_tb);

        rst_n = 0;
        repeat(20) @(posedge clk_4x);
        #1; rst_n = 1;

        for (int t = 0; t < 3000 && u_top.u_eu.u_rf.d_reg[3] !== 32'h3333_3333; t++)
            @(posedge clk_4x);
