...
// Timing-diagram source testbench: MC68030UM.pdf Figure 7-26's own
// word-write/byte-write/byte-write chain, driven through the REAL EU/
// decode pipeline (mirrors tb/read_cycle_eu_tb.sv exactly). With the
// post-Track-3 write-as-CURRENT preview fix (wobbly-honking-cascade.md),
// this chain now also closes with zero idle gap, matching the manual.

module manual_726_tb;
    ...
    initial begin
        ...
        $readmemh("../tests/timing_manual_726.hex", rom);
    end
    ...
    initial begin
        $dumpfile("manual_726.vcd");
        $dumpvars(0, manual_726_tb);

        rst_n = 0;
        repeat(20) @(posedge clk_4x);
        #1; rst_n = 1;

        for (int t = 0; t < 3000 && u_top.u_eu.u_rf.d_reg[3] !== 32'h1000_5678; t++)
            @(posedge clk_4x);
