...
81. "3rd port" investigation (`port3.md`). Diagnostic found the "(d8,An,Xn) indexed-dst arch gap" label was overbroad: re-ran AND.b (uses the identical 2-port `dyn_bit_get_Dn` time-multiplex trick BCHG's broken indexed form uses) → 100% pass, proving the 2-port scheme works fine for at least that shape of instruction. `port3.md` splits the gap into 4 buckets; Phase 81 implements **Bucket A** — CLR/NEG/NOT/NEGX/TST/TAS/memory-word-shift are unary memory ops needing only `An`+`Xn` (2 ports, like LEA/PEA indexed already does) — added `(d8,An,Xn)` decode (+ `(d16,An)`/abs.W/abs.L for shifts, which had no extended-EA support at all) to the 3 shared eu_seq.sv blocks, with matching `ext_count` entries in m68030_seq.sv. **No register-file port added.** Results: CLR.b/NEG.w/NOT.b/TST.b/TAS/ASL.w all → **100%**, zero remaining fails of any kind. See `plan.md §Phase 81` and `port3.md` for the full bucket breakdown and remaining open questions (Bucket C: BCHG/BCLR/BSET/MOVE — likely an isolated bug, not a port issue, per the AND.b evidence; Bucket D: CHK indexed — genuinely needs a 3rd register simultaneously, still the one confirmed case for the port).

**Current state**: 32/32 regression tests pass (`make test`). All 8 opcode groups pass vs Musashi (`make cosim_grp`). 50/50 synthetic dat-replay vectors pass (`make dat-synth`). Harte results: ADD/SUB/AND/OR/EOR/CMP b/w/l all 100%; MOVE.b/w/l 91–94%; BCHG/BCLR/BSET 92.8/93.4/98.2%; CHK 64.3% (zero logic mismatches, rest = unimplemented EA modes incl. indexed); CLR.b/NEG.w/NOT.b/TST.b/TAS/ASL.w all **100%** (Phase 81 — no port needed); NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/other-shift-sizes share the same fixed decode block and should also be 100% but weren't individually swept. Remaining non-100% suites (MOVE, BCHG/BCLR/BSET, CHK) need either a targeted bug fix (BCHG/BCLR/BSET/MOVE — see `port3.md` Bucket C) or the 3rd register-file port (CHK indexed only — Bucket D). See `plan.md §Phase 81` for full table.

**Architectural gap — narrower than previously documented**: only CHK's indexed form (`(d8,An,Xn)`, tested value + EA base + EA index all needed at once, no natural 2-phase read available) is confirmed to need a 3rd register-file read port. BCHG/BCLR/BSET/MOVE-reg-src indexed-dst are confirmed broken but likely an isolated bug in their RMW path, not a port-count limitation (AND/OR/EOR/SUB/CMP use the identical 2-port time-multiplex mechanism and pass 100%). See `port3.md` for the full 4-bucket analysis and design for adding the port if/when needed.

**Next phase**: Phase 82 — per `port3.md`'s recommended sequencing: (0.5) sweep OR/EOR/SUB/CMP.b and ADDA/SUBA/CMPA.w indexed forms to confirm they match AND's 100%; (0.75) root-cause BCHG/BCLR/BSET's indexed-dst failure via waveform trace (compare against a working AND-indexed vector); then decide whether the 3rd port is still worth building for Bucket C or only needed for CHK (Bucket D). Also: sweep remaining NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/shift-size suites to confirm 100%, BTST retest, MOVEQ 4 TIMEOUTs, and ADDA/SUBA/CMPA/Scc/MOVEM/MOVEP/MOVEA/LEA/PEA once sweep completes.
