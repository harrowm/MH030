...
**Current state**: 32/32 regression tests pass (`make test`). All 8 opcode groups pass vs Musashi (`make cosim_grp`). 50/50 synthetic dat-replay vectors pass (`make dat-synth`). Harte results: ADD/SUB/AND/OR/EOR/CMP b/w/l all 100%; MOVE.b/w/l/q all **100%** (Phase 85/89); BCHG/BCLR/BSET/BTST all **100%** (Phase 83/88); **CHK all 100%** (Phase 86); CLR/NEG/NOT/NEGX/TST all sizes **100%** (Phase 81/87); **ASL/ASR/LSL/LSR/ROL/ROR/ROXL/ROXR all sizes 100%** except ASL.b's 2 confirmed corpus anomalies (Phase 87). **The 3rd-register-file-port investigation (`port3.md`) is concluded — nothing in the codebase requires it.** **Phase 90 sweep found a large cluster of unresolved gaps** (BCD ops, MULS/MULU/DIVS/DIVU, Scc, LEA, PEA, MOVEM.l, MOVEtoSR — see item 90 above) plus JMP/JSR's residual indexed-EA timeout, none yet root-caused.
...
**Next phase**: Phase 91 — root-cause the Phase 90 cluster, roughly in order of severity: NBCD/SBCD/ABCD (likely share one root cause in `eu_bcd.sv`), MULS/MULU/DIVS/DIVU (likely share one root cause in `eu_mul_div.sv`), Scc, PEA, LEA, MOVEM.l, MOVEtoSR, then the JMP/JSR indexed-EA IFU-redirect stall. TRAP/RTE/RTR supervisor-state harness limitation remains separately tracked (Phase 92).
