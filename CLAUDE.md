...
**Next phase**: JMP/JSR's own indexed-EA timeout (now understood to be a *different* bug than MUL/DIV's — likely a genuine missing odd-jump-target Address Error trap, worth its own investigation), then Scc, PEA, LEA, MOVEM.l, MOVEtoSR. TRAP/RTE/RTR supervisor-state harness limitation remains separately tracked.
