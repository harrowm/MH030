...
**Current state**: 32/32 regression tests pass (`make test`). All 8 opcode groups pass vs Musashi (`make cosim_grp`). 50/50 synthetic dat-replay vectors pass (`make dat-synth`). Harte results: ADD/SUB/AND/OR/EOR/CMP b/w/l all 100%; MOVE.b/w/l 97.9/98.7/99.0% (Phase 82 — no port needed, remaining = deferred indexed-src); BCHG/BCLR/BSET 92.8/93.4/98.2%; CHK 64.3% (zero logic mismatches, rest = unimplemented EA modes incl. indexed); CLR.b/NEG.w/NOT.b/TST.b/TAS/ASL.w all **100%** (Phase 81 — no port needed); NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/other-shift-sizes share the same fixed decode block and should also be 100% but weren't individually swept. Remaining non-100% suites (BCHG/BCLR/BSET, CHK, MOVE indexed-src) need either a targeted bug fix (BCHG/BCLR/BSET — see `port3.md` Bucket C) or the 3rd register-file port (CHK indexed — Bucket D) or new src/dst Xn field infra (MOVE indexed-src, `port3.md` Phase X remainder). See `plan.md §Phase 82` for full table.
...
**Next phase**: Phase 83 — root-cause BCHG/BCLR/BSET's indexed-dst failure via waveform trace (compare against a working AND-indexed vector — `port3.md` Phase 0.75), then decide whether the 3rd port is still worth building for Bucket C or only needed for CHK (Bucket D). Also: sweep remaining NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/shift-size suites to confirm 100%, BTST retest, MOVEQ 4 TIMEOUTs, and Scc/MOVEM/MOVEP/MOVEA/LEA/PEA once sweep completes.
