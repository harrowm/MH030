...
**Next phase**: Phase 86 — CHK's remaining `(An)+`/`-(An)`/`(xxx).L`/`(d16,PC)`/`(d8,PC,Xn)` EA modes; sweep remaining NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/shift-size suites to confirm 100%; BTST retest; MOVEQ 4 TIMEOUTs; Scc/MOVEM/MOVEP/MOVEA/LEA/PEA once sweep completes.
