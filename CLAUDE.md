...
**Current state**: 32/32 regression tests pass (`make test`). All 8 opcode groups pass vs Musashi (`make cosim_grp`). 50/50 synthetic dat-replay vectors pass (`make dat-synth`). Harte results: ADD/SUB/AND/OR/EOR/CMP b/w/l all 100%; MOVE.b/w/l 97.9/98.7/99.0% (Phase 82 — no port needed, remaining = deferred indexed-src, `(d8,An,Xn)`/`(d8,PC,Xn)`); BCHG/BCLR/BSET all **100%** (Phase 83); CHK 70.3% (zero remaining failures on the indexed form, Phase 84 — rest = other unimplemented EA modes); CLR.b/NEG.w/NOT.b/TST.b/TAS/ASL.w all **100%** (Phase 81); NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/other-shift-sizes share the same fixed decode block and should also be 100% but weren't individually swept. **The 3rd-register-file-port investigation (`port3.md`) is concluded — nothing in the codebase requires it.** Remaining EA-mode gaps (MOVE indexed-src, CHK's other memory forms) are ordinary missing-decode work, unrelated to the port question.
...
**Next phase**: Phase 85 — add the remaining MOVE `(d8,An,Xn)`/`(d8,PC,Xn)` indexed-src modes (needs separate src/dst Xn scale fields, `port3.md` Phase X remainder) and CHK's remaining `(An)+`/`-(An)`/`(xxx).L`/`(d16,PC)`/`(d8,PC,Xn)` EA modes; sweep remaining NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/shift-size suites to confirm 100%; BTST retest; MOVEQ 4 TIMEOUTs; Scc/MOVEM/MOVEP/MOVEA/LEA/PEA once sweep completes.
