...
**Current state**: 32/32 regression tests pass (`make test`). All 8 opcode groups pass vs Musashi (`make cosim_grp`). 50/50 synthetic dat-replay vectors pass (`make dat-synth`). Harte results: ADD/SUB/AND/OR/EOR/CMP b/w/l all 100%; MOVE.b/w/l all **100%** (Phase 85); BCHG/BCLR/BSET all **100%** (Phase 83); **CHK all 100%** (Phase 86); CLR.b/NEG.w/NOT.b/TST.b/TAS/ASL.w all **100%** (Phase 81); NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/other-shift-sizes share the same fixed decode block and should also be 100% but weren't individually swept. **The 3rd-register-file-port investigation (`port3.md`) is concluded — nothing in the codebase requires it.**
...
**Next phase**: Phase 87 — sweep remaining NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/shift-size suites to confirm 100%; BTST retest; MOVEQ 4 TIMEOUTs; Scc/MOVEM/MOVEP/MOVEA/LEA/PEA once sweep completes.
