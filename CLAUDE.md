...
**Next phase**: Phase 93 — DIVS/DIVU from-scratch investigation (register-direct forms fail, so start there before even looking at memory-EA), then Scc, PEA, LEA, MOVEM.l, MOVEtoSR, then the shared JMP/JSR/MULS/MULU indexed-EA timeout investigation. TRAP/RTE/RTR supervisor-state harness limitation remains separately tracked.
