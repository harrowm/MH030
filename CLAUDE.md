...
**Current state**: 32/32 regression tests pass (`make test`). All 8 opcode groups pass vs Musashi (`make cosim_grp`). 50/50 synthetic dat-replay vectors pass (`make dat-synth`). Harte results: ADD/SUB/AND/OR/EOR/CMP b/w/l all 100%; MOVE.b/w/l 97.9/98.7/99.0% (Phase 82 — no port needed, remaining = deferred indexed-src); BCHG/BCLR/BSET all **100%** (Phase 83 — test-harness bug, zero RTL); CHK 64.3% (zero logic mismatches, rest = unimplemented EA modes incl. indexed); CLR.b/NEG.w/NOT.b/TST.b/TAS/ASL.w all **100%** (Phase 81 — no port needed); NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/other-shift-sizes share the same fixed decode block and should also be 100% but weren't individually swept. Only two things remain non-100%: CHK's indexed form (Bucket D, `port3.md`) and MOVE's indexed-*source* forms (`port3.md` Phase X remainder) — neither has been shown to need the 3rd port either; CHK in particular should be tried with the existing deferred-register pattern first (see below).

**Architectural gap — turned out not to exist (so far).** Every "needs a 3rd port" diagnosis investigated this session (Bucket A, Bucket C, most of MOVE) turned out to be missing decode or a test-harness bug, not an RTL port-count limitation — Bucket B (AND/OR/EOR/SUB/CMP/ADDA/SUBA/CMPA) already worked with the existing 2-port time-multiplex trick. Only CHK's indexed form (Bucket D) remains as a plausible candidate, and even that has a strong case for being closable the same way as Bucket B (the tested-value register isn't needed until after the memory read completes, so it can defer to the existing `dyn_bit_get_Dn` swap instead of needing a true 3rd simultaneous read) — this hasn't been attempted yet. See `port3.md` for the full analysis; the honest current state is that nothing has been *shown* to require the register-file port.

**Next phase**: Phase 84 — try CHK indexed with the existing `dyn_bit_get_Dn` deferred-register pattern (`port3.md` §1 Bucket D, revised; §2 step 5) before considering the port at all. Also: add the remaining MOVE `(d8,An,Xn)`/`(d8,PC,Xn)` indexed-src modes (needs separate src/dst Xn scale fields, `port3.md` Phase X remainder); sweep remaining NEGX/NOT.w-l/CLR.w-l/NEG.b-l/TST.w-l/shift-size suites to confirm 100%; BTST retest; MOVEQ 4 TIMEOUTs; Scc/MOVEM/MOVEP/MOVEA/LEA/PEA once sweep completes.
